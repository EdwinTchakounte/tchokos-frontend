...
import { getZones, registerCourier, type CourierZone } from "@/lib/courier";
...
export default function CourierAuthPage() {
  ...
  const { ready, isCourier } = useAuth();
  const [mode, setMode] = useState<Mode>("login");

  useEffect(() => {
    if (ready && isCourier) router.replace("/livreur/courses");
  }, [ready, isCourier, router]);
        ...
        <div className="p-6 sm:p-8">
          ...
          {mode === "login" ? (
            <div className="space-y-4">
              <h3 className="font-display text-lg font-bold text-ink">Déjà livreur ?</h3>
              <p className="text-sm text-slate-500">
                Connectez-vous avec votre email et mot de passe (ou par code SMS),
                puis accédez à vos courses.
              </p>
              <Link href="/connexion?next=/livreur/courses"
                className="block w-full rounded-full bg-brand-600 px-6 py-3 text-center font-semibold text-white hover:bg-brand-700">
                Se connecter
              </Link>
            </div>
          ) : (
            <RegisterForm onDone={() => router.replace("/livreur/courses")} />
          )}
        </div>
      </div>
    </div>
...
function RegisterForm({ onDone }: { onDone: () => void }) {
  ...
  const [form, setForm] = useState({ name: "", email: "", password: "", phone: "", vehicle: "Moto" });
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <input required type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="Mot de passe (8 car. min.) *" className={input} />
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {zones.map((z) => (
            <button type="button" key={z.id} onClick={() => toggleZone(z.id)}
              className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${selected.includes(z.id) ? "bg-brand-600 text-white" : "bg-slate-100 text-ink-soft hover:bg-slate-200"}`}>
              {z.name}
            </button>
          ))}
        </div>
